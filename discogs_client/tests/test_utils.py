...
class UtilsTestCase(DiscogsClientTestCase):
    def test_update_qs(self):
        ...
        u = utils.update_qs
        self.assertEqual(
            u("http://example.com", {"foo": "bar"}), "http://example.com?foo=bar"
        )
        self.assertEqual(
            u("http://example.com?foo=bar", {"foo": "baz"}),
        ...
        self.assertEqual(
            u("http://example.com", {"a": "t\xe9st"}), "http://example.com?a=t%C3%A9st"
        )
    ...
    def test_backed_off_when_rate_limit_reached(self, patched_duration):
        ...
        self.assertEqual(
            mock_ok_response, test_class.returns_non_ratelimit_status_code()
        )
