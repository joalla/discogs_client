...
class Client:
    ...
    def get_authorize_url(self, callback_url=None):
        ...
        if status_code != 200:
            raise AuthorizationError(
                "Could not get request token.", status_code, content
            )
    ...
    def get_access_token(self, verifier):
        ...
        content, status_code = self._fetcher.fetch(
            self, "POST", self._access_token_url, headers=params
        )
        if status_code != 200:
    ...
    def search(self, *query, **fields):
        ...
        return models.MixedPaginatedList(
            self, update_qs(self._base_url + "/database/search", fields), "results"
        )
    ...
    def fee_for(self, price, currency="USD"):
        ...
        resp = self._get(
            "{0}/marketplace/fee/{1:.4f}/{2}".format(self._base_url, price, currency)
        )
        return models.Price(
            self, {"value": resp["value"], "currency": resp["currency"]}
        )
    ...
    def set_timeout(
        self, connect: Union[int, float] = 5, read: Union[int, float] = 10
    ) -> None:
